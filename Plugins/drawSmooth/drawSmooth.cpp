...
{  
    ...
    vector <Vector> normalsVertex(objecteActual.vertices().size());

    for (int i = 0;i < (int)cares.size();++i) {
        const Face &cara = cares[i];
        for (int j = 0;j < (int)cara.numVertices();++j) {
            int index = cara.vertexIndex(j);
            normalsVertex[index] += cara.normal();
            indices.push_back(index);
        }
    }

    for (int i = 0;i < (int)vertexos.size();++i) {

        Vector N = normalsVertex[i].normalized();
        Point P = vertexos[i].coord();
